...
<div class="container">
    ...
    @if(isset($societe->logo) && $societe->logo)
        <div class="logo-container">
            @php
                $logoPath = public_path('storage/' . $societe->logo);
                $logoExists = file_exists($logoPath);
                $logoExtension = $logoExists ? strtolower(pathinfo($logoPath, PATHINFO_EXTENSION)) : '';
            @endphp

            @if($logoExists && in_array($logoExtension, ['jpg', 'jpeg']))
                <img src="{{ public_path('storage/' . $societe->logo) }}" alt="شعار الشركة" class="logo">
            @else
                <div class="company-name-fallback">{{ $societe->societe_nom ?? $societe->nom ?? 'الشركة' }}</div>
            @endif
        </div>
    @endif
