          ...
          <div class="page-title-section">
            <div class="title-wrapper">
              ...
              <div class="title-content">
                ...
                <p class="page-subtitle">Gérez vos résiliations de contrats de burreaux</p>
              </div>
            </div>
          </div>
                ...
                <tbody>
                  <tr v-for="record in filteredData" :key="record.id" class="table-row">
                    <td class="reference-cell">
                      <span class="reference-badge">{{ record.bureau_equipe?.reference_numero || "-" }}</span>
                    </td>
                    <td class="date-cell">
                      <i class="fas fa-calendar-alt me-1"></i>
                    ...
                    <td>
                      <div class="action-buttons">
                        <button class="action-btn download-action" @click="downloadContract(record)"
                          :disabled="isSubmitting && downloadingId === record.id" title="Télécharger PDF">
                          <i class="fas fa-download" v-if="!isSubmitting || downloadingId !== record.id"></i>
    ...
    filteredData() {
      ...
      if (s) {
        filtered = filtered.filter(
          (r) =>
            (r.bureau_equipe?.reference_numero && r.domiciliation.reference_numero.toLowerCase().includes(s)) ||
      ...
      if (this.filters.reference) {
        filtered = filtered.filter(r =>
          r.bureau_equipe?.reference_numero && r.domiciliation.reference_numero.toLowerCase().includes(this.filters.reference.toLowerCase())
        );
      }
  ...
  methods: {
    ...
    exportToExcel() {
      const dataToExport = this.allFilteredData.map((res) => ({
        Référence: res.bureau_equipe?.reference_numero || "-",
    ...
    // Dans les méthodes du composant ResiliationsManagement
    exportToPDF() {
      if (!this.companyInfo) {
      ...
      printWindow.document.title = "Liste des résiliations de bureaux";
      printWindow.focus();
    },
...
<html>
  <head>
    <title>Liste des résiliations de bureaux</title>
    <meta charset="UTF-8">
  ...
  <body>
      ...
      <table>
        ...
        <tbody>
          ...
          .map(
            (res) => `
            <tr>
              <td>${res.bureau_equipe?.reference_numero || '-'}</td>
              <td>${this.formatDate(res.date_resiliation)}</td>
              <td>${this.getRaisonText(res.raison)}</td>
              <td class="text-center">
                <span class="badge ${this.getStatusClass(res.status)}">
    ...
    <script>
      window.onload = function() {
        document.title = "Liste des résiliations de bureaux";
      }
    <\/script>
  </body>
</html>`;
    ...
    generatePDFWithoutLogo() {
      ...
      doc.setFontSize(18);
      doc.text("Liste des résiliations de bureaux", 14, 20);
      doc.setFontSize(11);
      ...
      doc.autoTable({
        ...
        body: this.allFilteredData.map((res) => [
          res.bureau_equipe?.reference_numero || '-',
      ...
      doc.save(`resiliations_bureaux_${new Date().toISOString().split("T")[0]}.pdf`);
    },
    ...
    async downloadContractPDF(id) {
      try {
        const response = await axios.get(`/bureau/resiliations/resiliationContractPDF/${id}`, {
    ...
    async downloadContractWORD(id) {
      try {
        const response = await axios.get(`/bureau/resiliations/resiliationContractWORD/${id}`);
        ...
        const dateCreation = formatDate(bureau?.created_at) || "N/A"
        const dateFin = formatDate(bureau?.date_fin) || "N/A";
              ...
              children: [
                ...
                new Paragraph({
                  children: [
                    ...
                    new TextRun({
                      text: "Résiliation de bureau équipé",
                      bold: true,
                ...
                new Paragraph({
                  children: [
                    ...
                    new TextRun({
                      text: `, gratuitement une attestation de bureau équipé sur notre adresse suivante : `,
                      size: 22,
                      color: "000000",
                    }),
                    new TextRun({
                ...
                new Paragraph({
                  children: [
                    ...
                    new TextRun({
                      text: `, nous décidons de mettre fin à cette bureau équipé.`,
                      size: 22,
                      color: "000000",
                    }),
                  ],
    ...
    async onDeleteResiliation(record) {
      ...
      if (result.isConfirmed) {
        try {
          // Changé de route
          await axios.delete(`/resiliations-bureaux/${record.id}`, {
            headers: this.getApiHeaders(),
          });
    ...
    async fetchResiliations() {
      try {
        const res = await axios.get("/resiliations-bureaux", {
          headers: {
